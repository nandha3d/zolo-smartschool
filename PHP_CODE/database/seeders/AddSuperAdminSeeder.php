...
class AddSuperAdminSeeder extends Seeder {
    ...
    public function run() {
        ...
        // The password was hardcoded as 'superadmin'. It is now generated, printed
        // once here, and the account is held at the change-password screen until it
        // is replaced. Set SUPER_ADMIN_EMAIL / SUPER_ADMIN_PASSWORD to pin them for
        // an automated deployment.
        $email = env('SUPER_ADMIN_EMAIL', 'admin@zoloschools.com');
        $plainPassword = env('SUPER_ADMIN_PASSWORD') ?: \App\Services\PasswordService::generate(16);
        ...
        $user = User::updateOrCreate(['id' => 1], [
            ...
            'password'   => $existing->password ?? Hash::make($plainPassword),
            'must_change_password' => $existing->must_change_password ?? 1,
            'gender'     => 'male',
            'image'      => 'logo.svg',
            'mobile'     => "",
        ...
        // Only announce credentials when this run actually created the account, so
        // re-seeding an existing installation never prints a password that is wrong.
        if (!$existing) {
            $this->command?->newLine();
            $this->command?->warn('Super admin created. This password is shown once:');
            $this->command?->line('  email    : ' . $email);
            $this->command?->line('  password : ' . $plainPassword);
            $this->command?->warn('You will be asked to change it on first sign in.');
            $this->command?->newLine();
        }
...
//        ]);
        ...
        SystemSetting::upsert([
            ...
            ["name" => "system_name", "data" => "Zolo Schools", "type" => "string"],
            ["name" => "address", "data" => "Zolo Schools", "type" => "string"],
            ...
            ["name" => "short_description", "data" => "Zolo Schools - Manage Your School", "type" => "text"],
            ["name" => "facebook", "data" => "https://www.facebook.com/zoloschools", "type" => "text"],
            ["name" => "instagram", "data" => "https://www.instagram.com/zoloschools", "type" => "text"],
            ["name" => "linkedin", "data" => "https://www.linkedin.com/company/zoloschools", "type" => "text"],
            ["name" => "footer_text", "data" => "<p>&copy;&nbsp;<strong>Zolo Schools</strong>. All Rights Reserved</p>", "type" => "text"],
        ...
        $systemSettings = [
            [
                'name' => 'hero_title_1',
                'data' => 'Opt for Zolo Schools 14+ robust features for an enhanced educational experience.',
                'type' => 'text'
            ],
            [
                'name' => 'hero_title_2',
            ...
            [
                'name' => 'tag_line',
                'data' => 'Transform School Management With Zolo Schools',
                'type' => 'text'
            ],
            [
                'name' => 'mobile',
            ...
            [
                ...
                'data' => 'Experience the future of education with our Zolo Schools platform. Streamline attendance, assignments, exams, and more. Elevate your school\'s efficiency and engagement.',
                'type' => 'text'
            ],
            [
