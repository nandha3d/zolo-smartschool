...
function changeEnv($data = array()) {
    if (count($data) > 0) {
        ...
        // Split on any line ending. This used PHP_EOL, which on Windows is "\r\n" and
        // therefore failed to split an LF-terminated .env - the whole file became one
        // element, no key ever matched, and every write appended a duplicate instead
        // of replacing. The shipped .env already carried QUEUE_CONNECTION twice.
        $env = preg_split('/\r\n|\r|\n/', $env);
...
function resizeImage($image) {
    // Intervention v3: the target path is explicit and quality is a named argument.
    ImageService::manager()->read($image)->save($image, quality: 50);
}
