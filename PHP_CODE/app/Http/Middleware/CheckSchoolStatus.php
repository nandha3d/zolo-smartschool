...
class CheckSchoolStatus {
    ...
    public function handle(Request $request, Closure $next) {
        ...
        if (strpos($url, 'api') !== false) {
            // Tenant selection and authentication are already done, by
            // APISwitchDatabase followed by auth:sanctum. This middleware used to
            // repeat both, looking the bearer token up by hand and calling
            // Auth::loginUsingId() - which now fails outright, because with
            // auth:sanctum the active guard is a RequestGuard and has no such method.
            if (!$request->user()) {
