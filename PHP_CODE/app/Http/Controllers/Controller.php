...
class Controller extends BaseController
{
    ...
    /**
     * These both returned a value derived from data the school already holds - the
     * guardian's mobile number, and the student's date of birth as ddmmyyyy. Neither
     * is secret, so both now return a generated password. Arguments are accepted and
     * ignored so existing call sites are unaffected.
     */
    public function makeParentPassword(...$ignored)
    {
        return \App\Services\PasswordService::generate();
    }

    public function makeStudentPassword(...$ignored)
    {
        return \App\Services\PasswordService::generate();
    }
    ...
    public function contact(Request $request)
    {
        try {
            ...
            // Previously dd($e) - that rendered a full stack trace to an unauthenticated
            // visitor and made the handling below unreachable.
            Log::error('Contact form failed', ['exception' => $e->getMessage()]);
    ...
    // relatedDataIndex()/relatedDataDestroy() and their two query helpers were
    // removed. They took a table name straight from the URL: the destroy route
    // deleted any row in any table with no permission check, and the index route
    // concatenated that same name into a DB::raw() select clause. The feature was
    // already unreachable from the UI - the links in related-data/index.blade.php
    // are commented out.
    ...
    public function school_db_test()
    {
        // return 1;
        // return $request;
        // Config::set('database.connections.school.database', 'zolo_schools_2_school');
        // DB::purge('school');
        // DB::connection('school')->reconnect();
        // DB::setDefaultConnection('school');
        return Auth::user();
