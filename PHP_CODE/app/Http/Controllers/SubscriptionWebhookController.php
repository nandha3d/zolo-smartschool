...
class SubscriptionWebhookController extends Controller
{
    ...
    public function stripe(Request $request)
    {
        ...
        try {
            ...
            // Log the event identity only. The full event body carries payment and
            // customer detail that must not be written to application logs.
            Log::info('Stripe Webhook Event', ['id' => $event->id, 'type' => $event->type]);
    ...
    /**
     * Razorpay signs the raw request body with the webhook secret and sends the digest
     * in the X-Razorpay-Signature header. Until that digest is verified the body is
     * attacker-controlled, so this must run before any transaction is looked at.
     */
    private function razorpaySignatureIsValid(Request $request): bool
    {
        $signature = $request->header('X-Razorpay-Signature');
        if (empty($signature)) {
            Log::warning('Razorpay Webhook : rejected, no signature header');
            return false;
        }

        DB::setDefaultConnection('mysql');
        $paymentConfiguration = PaymentConfiguration::where('payment_method', 'razorpay')
            ->whereNull('school_id')
            ->first();

        if (empty($paymentConfiguration->webhook_secret_key)) {
            Log::error('Razorpay Webhook : rejected, webhook secret is not configured');
            return false;
        }

        try {
            $api = new Api(
                $paymentConfiguration->webhook_public_key,
                $paymentConfiguration->webhook_secret_key
            );
    ...
    {
        if (!$this->razorpaySignatureIsValid($request)) {
        ...
        try {
            ...
            DB::setDefaultConnection('mysql');
            $paymentConfiguration = PaymentConfiguration::where('payment_method', 'razorpay')->whereNull('school_id')->first();
            ...
            // $metadata = $data->payload->payment->entity->notes;
            // metadata intentionally not logged: contains payment identifiers
