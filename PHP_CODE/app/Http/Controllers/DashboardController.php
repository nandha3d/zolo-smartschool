    ...
    public function index()
    {
        ...
        if (Auth::user()->hasRole('Super Admin') || Auth::user()->school_id == null) {
            $server_configuration = [
                'database_root_user' => [ 'title' => trans('database_root_user_access'), 'link' => 'https://docs.zoloschools.com/installation/admin-panel-setup/vps-server-setup/#9%EF%B8%8F%E2%83%A3-get-mysql-root-credentials','description' => trans('Required for creating and dropping school-specific databases in this SaaS system. When a new school is added, the root user permissions ensure database creation, migrations, and default setup run automatically.')],
                'laravel_queue_setup' => [ 'title' => trans('background_task_processing'), 'link' => 'https://docs.zoloschools.com/installation/queue-setup/', 'description' => trans('The school creation process takes 2–3 minutes due to database setup and migrations. Laravel queues handle these tasks in the background so the system remains responsive.')],
                'wildcard_domain' => [ 'title' => trans('subdomain_support'), 'link' => 'https://docs.zoloschools.com/installation/admin-panel-setup/vps-server-setup/#5%EF%B8%8F%E2%83%A3-add-wildcard-domain', 'description' => trans('Enables each school to have its own subdomain (e.g., schoolname.yourdomain.com). Needed if schools want their own branded web portal.')],
                'web_socket_setup' => [ 'title' => trans('real_time_communication'), 'link' => 'https://docs.zoloschools.com/installation/admin-panel-setup/vps-server-setup/#-set-up-websocket', 'description' => trans('Required for mobile app real-time chat and instant updates. WebSockets ensure fast, bidirectional data exchange.')],
                'notification_settings' => [ 'title' => trans('push_notification'), 'link' => 'https://docs.zoloschools.com/superadmin/system-settings/notification-settings/', 'description' => trans('Enables app push notifications using Firebase Cloud Messaging (FCM) for alerts, updates, and reminders.')],
            ];
